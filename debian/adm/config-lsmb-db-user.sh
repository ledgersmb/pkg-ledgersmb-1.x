...
BEGIN
    ...
    IF num_users = 0 THEN
        CREATE ROLE $LSMBDBUSER WITH CREATEDB LOGIN NOINHERIT ENCRYPTED PASSWORD '$LSMBDBPW';
    ELSE
        ALTER ROLE $LSMBDBUSER WITH CREATEDB LOGIN NOINHERIT ENCRYPTED PASSWORD '$LSMBDBPW';
    END IF;
END
\$\$
;
